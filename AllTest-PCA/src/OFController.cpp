...
void OFColor::setColor(const int &colorCode) {
    ...
    if ((R + G + B) > 0) {
        ...
        r_cal *= a * Config::OF_MAX_BRIGHTNESS_R;
        g_cal *= a * Config::OF_MAX_BRIGHTNESS_G;
        b_cal *= a * Config::OF_MAX_BRIGHTNESS_B;

        r_cal = pow(r_cal, 1.0 / Config::GAMMA_R);
        g_cal = pow(g_cal, 1.0 / Config::GAMMA_G);
        b_cal = pow(b_cal, 1.0 / Config::GAMMA_B);
